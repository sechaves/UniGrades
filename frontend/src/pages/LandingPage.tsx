...
function Feature({ icon, title, desc }: { icon: React.ReactNode; title: string; desc: string }) {
  return (
    <div className="flex flex-col items-start gap-3 rounded-xl border border-gray-200 dark:border-gray-800 p-6">
      <div className="p-3 rounded-lg bg-brand-50">{icon}</div>
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-gray-950">
      ...
      <nav className="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-gray-900 max-w-6xl mx-auto">
        <img src="/logo.png" alt="UniGrades" className="h-8 w-auto" />
      ...
      <section className="max-w-4xl mx-auto px-6 py-24 text-center">
        <h1 className="text-5xl font-extrabold text-gray-900 dark:text-gray-100 leading-tight mb-6">
          ...
          <span className="text-brand-500">organizado.</span>
        </h1>
        ...
        <div className="flex justify-center gap-4">
          <Link
            to="/register"
            className="rounded-xl bg-brand-500 px-6 py-3 text-base font-semibold text-white hover:bg-brand-600 transition-colors"
          >
            Empezar gratis
          </Link>
          <Link
      ...
      <section className="max-w-5xl mx-auto px-6 pb-24">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
          ...
          />
          <Feature
            icon={<Award size={22} className="text-brand-600" />}
